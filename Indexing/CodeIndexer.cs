    ...
    ILogger<CodeIndexer> logger) : ICodeIndexer
{
    ...
    public async Task IndexFileAsync(string repoName, string filePath, CancellationToken ct = default)
    {
        ...
        var normalizedImports = language is ILanguageIndexingPlugin indexing
            ? indexing.NormalizeImports(project!.Config.ResolvedPath, filePath, allImports)
            : [.. allImports.Distinct()];
    ...
    private async Task RefreshReferencesForFileAsync(string repoName, string filePath, ILanguage language, CancellationToken ct)
    {
        ...
        if (language is ILanguageIndexingPlugin indexing)
        {
            var semanticRefs = await indexing.BuildSemanticReferencesAsync(
        ...
        var usages = language is ILanguageIndexingPlugin plugin
            ? plugin.ExtractUsages(filePath, lines)
