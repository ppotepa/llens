...
public class CSharpLanguage : ILanguage<CSharp>, ILanguageIndexingPlugin
{
    ...
    public IReadOnlyList<string> NormalizeImports(string repoRoot, string filePath, IReadOnlyList<string> imports)
        => [.. imports.Distinct(StringComparer.OrdinalIgnoreCase)];
    ...
    public async Task<IReadOnlyList<SymbolReference>> BuildSemanticReferencesAsync(LanguageReferenceContext context, CancellationToken ct)
    {
        var source = string.Join('\n', context.Lines);
