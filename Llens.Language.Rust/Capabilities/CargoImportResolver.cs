...
/// <summary>
/// Resolves Rust <c>use</c> imports to concrete file paths using Cargo module conventions.
/// Handles crate/self/super roots, nested group imports, workspace members, and path dependencies.
/// </summary>
public sealed class CargoImportResolver : IImportResolver<Rust>
{
    ...
    private static List<string>? GetCurrentModuleSegments(string srcRoot, string fullFilePath)
    {
        ...
        if (file.Equals("lib.rs", StringComparison.OrdinalIgnoreCase)
            ...
            || file.Equals("mod.rs", StringComparison.OrdinalIgnoreCase))
            return dirs;
    ...
    private static Dictionary<string, string> ReadPathDependencies(string crateRoot)
    {
        ...
        foreach (var raw in File.ReadLines(cargoToml))
        {
            ...
            var match = Regex.Match(line, @"^(?<name>[A-Za-z0-9_\-]+)\s*=\s*\{[^}]*path\s*=\s*""(?<path>[^""]+)""", RegexOptions.IgnoreCase);
